...
We can compare this the results of Figure~\ref{fig:txbParastichyCountsTo89} below to see that, depending on the size of the initial disk radius, a scaling of  around 89 is needed to transition to a parastichy pair of (55,89).  
...
However when analysing the MOSI images, that paper called attention to a previously unreported departure from Fibonacci structure, which we there called a lack of rotational symmetry~\cite{swintonNovelFibonacciNonFibonacci2016}. Perhaps surprisingly, the stacked-disk model for falling phyllotaxis, with increasing $f$ is also capable of illustrating this type of pattern, as illustrated in Figure~\ref{fig:txbFalling}, which 
