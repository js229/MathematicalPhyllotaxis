...
Disk-stacking models have now been shown to exhibit further empirical phenomena in ways no other models have yet achieved or seem likely to~\cite{swintonDiskstackingModelsAre2024}. However there remain many open questions about the dynamics of this important class of models.
}
...
\mmafig{Txb1004NonopposedNo}{Non-opposed lattices cannot in general be generated by stacked-disk models. }{1}
...
\mmafig{Txb1007ParastichyCountsTo89}{Transitions between parastichy counts as a function of the slowly changing inverse radius of the disks. $y$ axis: Local chain parastichy counts for the topmost chain through each successive disk in a single
	...
	The thin grey vertical lines display the theoretical prediction that the transition from a parastichy number of $F_{k-2}$ to $F_{k}$ should occur at $r{-1}=\sqrt{2}F_{k+2}$. 
}{1}
...
\mmafig{Txb1010Tilings}{Rhombic tilings arising from a stacked coin model.
	Tiles are coloured to emphasise a periodicity of 6 for each tiling after the first. }{.8}
\pdffig{F1011DouadyGole2016bFig4}{Fit of a lattice and a cylinder tiling to the same unrolled birch catkin image. The cylinder tiling is a 14\% better fit to the node positions than the lattice. From~\autocite{douadyFibonacciQuasisymmetricPhyllotaxis2016}.
...
To do this, of course, we need some data to compare, and as we have seen there all of the possible datasets so far available have their limitations. Couder's 1998 dataset of 21 sunflowers is notable for not only containing, by design, both Fibonacci and non-Fibonacci specimens, but for recording both leaf and seed patterns on the same specimen~\cite{couderInitialTransitionsOrder1998}. Couder showed moreover how this data was compatible with an energy-based model, of which stacked-disk models are a special case. 

 very few possibilities to choose 
