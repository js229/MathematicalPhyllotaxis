...
\index{cylindrical lattice!divergence}
\index{cylindrical lattice!rise}
...
The parastichy vector $\pvec{2}$, in red, is the shortest vector on the cylinder to the point \protect\jpoint{2} and always has $|x_2|\leq\jhalf$. The second-shortest vector to the point will wind in the opposite direction and is called the complementary vector $\phatvec{2}$, in blue. Here $d=17/72$ so $x_2=2d$ and $\phatvec{2}=\pvec{2}-\pvec{0}$ where  $\pvec{0}=(1,0)$ is the horizontal vector around the circumference of the cylinder. A further, un-named vector is shown in green that winds more than a full turn about the cylinder: there are infinitely many other vectors from periodic images of the point \protect\jpoint{0} to the point \protect\jpoint{2}, each with a non-zero winding numbers. 
}{1}
...
%	\label{ex:npm}
%\end{jExercise}
%\begin{% not copied to Answers
%jAnswer}{ex:pcongruent}{
...
\mmafig{Txb0405Parastichy5}{
	The family of 5-parastichies is a set of five lines on the cylinder, each member passing through one of the points \protect\jpoint{0} to \protect\jpoint{4}.
	The  line that passes through the origin  \protect\jpoint{0}, by definition the origin-parastichy of order 5,  also passes through the points \protect\jpoint{5} and all points $\protect\jpoint{5k}$. The parastichy vector $\pvec{5}$ (arrowed) is the shortest vector from $\protect\jpoint{0}$ to $\protect\jpoint{5}$. 
	This lattice has divergence $d=17/72$ and rise $h=0.03$.
}{1}


We model this psychological choice through the parastichy vector. For an integer $m$, the origin-parastichy of order $m$ is the infinite line on the cylinder winding through \jpoint{0} and $\jpoint{m}$ and  with slope $mh/\dm$ (or zero in the case $m=0$). This choice of slope is equivalent to choosing the line that traverses the smallest $x$ distance between $0$ and $\jpoint{m}$, and the portion of this line between $0$ and $\jpoint{m}$ coincides with the vector $\mvec=(\dm,mh)$ defined above as the parastichy vector.\index{parastichy lines}\index{parastichy lines!origin-parastichy}
...
We say that $\jpoint{m}$ is a \emph{visible point}, or the $m$-parastichy is a \emph{visible parastichy}\index{parastichy vector!visible vector}, if the latter is linearly independent of all the $n$-parastichies for all $|n|<|m|$. 
...
\index{pairs of parastichy vectors!opposed pair}
...
\index{pairs of parastichy vectors!generating pair}
...
\index{cylindrical lattice!golden}
...
\index{cylindrical lattice!golden angle divergence}
\begin{theorem}
...
\label{sec:ppair}
One lattice can have many generating, or generating and opposed, parastichy pairs.  We want to pick a principal pair\index{pairs of parastichy vectors!principal pair} that preserves the idea of being the `most obvious' to the eye. 
...
\index{parastichy number!principal}
\index{parastichy number!first two as lattice label}
\index{parastichy vector!principal vector}
\index{cylindrical lattice!classified by principal parastichies}
...
\index{cylindrical lattice!opposed}
\index{cylindrical lattice!non-opposed}

\section{Spiral lattices}
\label{sec:spiral}
\index{cylindrical lattice!spiral}
...
\index{Euclidean algorithm!reduction of generating pairs}
...
\mmafig{Txb0415SumDifference}{Turing's theorem: the third parastichy vector in a lattice is always the sum or difference of the first two. It is intuitively clear in this case (and ensured by the Theorem) that the point reached by third parastichy vector lies on the adjacent 4-parastichy to the origin, and so has paastichy number 9 or 5 depending on where the normal to the 4-parastichy through the origin intersects the adjacent parastichy.}{1.0}




\section{Orthostichies}
With the notation in place we can observe a consequence of having golden angle lattices. An \textit{orthostichy} is a parastichy\index{parastichy vector!orthostichy vector} vector of nonnegative rise which is closer to the vertical than any other parastichy vector of lower nonnegative rise.%
...
\index{cylindrical lattice!hexagonal}
\begin{definition}
...
\end{definition}
\index{cylindrical lattice!square}
...
\end{definition}
\index{cylindrical lattice!touching circle}
...
{1.0}
 ...
 %
\index{cylindrical lattice!multijugate} 
% %
% 
