...
The second column is the corresponding B\'ezout pair, as can be checked:
\begin{jExercise}
...
\end{jAnswer}
 Moreover the second column of each matrix is in fact the corresponding winding-number pair as we will see in the next section.
...
\end{align*}
and recalling that $F_{7}=13$, more generally
\begin{align}
	\begin{pmatrix} 
		F_{j+1} & F_{j-1} 
		\\
...
From \eqref{eq:FibonacciMatrix},
	\begin{align}		 
		...
		n  & v
			...
			= (-1)^{j-1} 
	\end{align}
so $(u,v)$ are a B\'ezout pair for $(m,n)$. Moreover the Farey interval mediant is
	\begin{align}		 
(u+v)/(m+n)&= F_j/F_{j+2}
	\end{align}
...
\begin{align}
	\label{eq:GeneralisedF}
		...
		F^k_2&=k.
\end{align}
so that Fibonacci numbers have $k=1$ and Lucas numbers $k=3$.
By inspecting Figure~\ref{fig:Ch2EuclideanTree}, remembering that $F^3_4=7$, or by induction, we can see that for $k>1$
\begin{align}
	\begin{pmatrix} 
		F^k_{j+1} & F_j 
		\\
...
\end{align}
So we see that the winding numbers for the generalised Fibonacci pair   and $F^k_{j+1}$ and $F^k_j$  are the plain Fibonacci pair $F_j$ and $F_{j+1}$, and the Farey interval of this matrix contains the Farey mediant
\begin{align}
	\frac{	 F_j + F_{j+1}}{F^k_{j+1}+F^k_{j-1}}&=	\frac{	 F_{j+2}}{F^k_{j+1}}
	\\
	&= 	\frac{	 F_{j+2}}{F_{j-1} + k F_j}
	\\&= \frac{ F_{j+2}/F_{j-1}}{1+ k F_j/F_{j-1}}
%	\\
%	&= \frac{	 F_{j+2}/{F_{j-1}}}{1+ k F_j/{F_{j-1}}}
\end{align}
making use of the well-known relation $	F^k_{j+1} =  F_{j-1} + k F_j$ and in the limit of large $j$ we find that the Farey interval for the matrix~\eqref{eq:GeneralizedFibonacciMatrix} contains the point
\begin{align}
\lim_{j\tends \inf}	F_j/F^k_{j+1}&= \tau/(1+k\tau)
\end{align} 
which we will  use later.


\begin{jExercise}
	Show that  $	F^k_{j+1} =  F_{j-1} + k F_j$
\end{jExercise}
\begin{jAnswer}
\end{jAnswer}
